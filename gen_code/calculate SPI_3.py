...
# ---------------------------------------------------------
OUT_GRID_NC = (
    r"H:\ZHUHAI WORKSHOP\SPI-3\NEW"
    r"\five_provinces_spi3_grid_1970_2022.nc"
)

OUT_PROV_CSV = (
    r"H:\ZHUHAI WORKSHOP\SPI-3\NEW"
    r"\five_provinces_spi3_provincial_1970_2022.csv"
)
...
def main():
    ...
    #
    # SPI-3 for January 1970 requires precipitation from
    # November and December 1969. Therefore, retain the
    ...
    if actual_month_index[0] != calculation_start_period:
        raise RuntimeError(
            ...
            "preceding months required to calculate SPI-3 "
    ...
    # -----------------------------------------------------
    # 7. Calculate grid SPI-3
    #
    # Core SPI logic is unchanged.
    # -----------------------------------------------------
    print(
        "\nComputing grid SPI-3 ..."
    )
    ...
    # to supply the SPI-3 rolling window. The final NetCDF
    ...
    if spi3_grid_crop.sizes["time"] != expected_month_count:
        raise RuntimeError(
            f"Expected {expected_month_count} SPI output "
            f"months between {TIME_START} and {TIME_END}, "
            f"but found {spi3_grid_crop.sizes['time']}."
        )
    ...
    spi3_grid_crop.name = "spi_3"

    spi3_grid_crop.attrs[
        "long_name"
    ] = (
        "SPI-3 over five selected provinces"
    )
    ...
    spi3_grid_crop.to_netcdf(
        OUT_GRID_NC
    )

    print(
        "Saved grid SPI-3 NetCDF:",
        OUT_GRID_NC,
    )

    # -----------------------------------------------------
    # 8. Provincial monthly precipitation and SPI-3
    # -----------------------------------------------------
    print(
        "\nComputing provincial monthly precipitation "
        "and provincial SPI-3 ..."
    )
    ...
    for province in PROVINCES:
        ...
        # Provincial SPI-3 calculated from the provincial
        ...
        # rolling window. Provincial precipitation and SPI-3
    ...
    print(
        "Shape:",
        spi3_grid_crop.shape,
    )

    grid_valid_count = int(
        np.isfinite(
            spi3_grid_crop.values
        ).sum()
    )
